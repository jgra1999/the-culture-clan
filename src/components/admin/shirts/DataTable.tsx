...
const headers = ['Color', 'Talla', 'Referencia', 'Stock']

export function ShirtsTable() {
	const [data, setData] = useState<Shirt[] | null>([])
	...
	return (
		<>
			<div className='flex flex-col md:flex-row gap-y-5 w-full md:items-center justify-between my-10'>
				<a
					href='/admin/franelas/agregar-franela'
						...
						<>
							{data.map((item: Shirt) => (
								<tr key={item.id} className='border-b border-mediumGray '>
									<th className='px-6 py-4'>{item.color}</th>
