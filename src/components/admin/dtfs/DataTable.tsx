...
import type { DTF } from '@/types'

const headers = ['Diseño', 'Colección', 'Stock']

export function DTFSTable() {
	const [data, setData] = useState<DTF[] | null>([])
	...
	const deleteRegister = async (id: string) => {
		...
		if (error) {
			toast.custom(<ErrorToast message='No se pudo eliminar el registro' />)
		} else {
			toast.custom(<SuccessToast message='Registro eliminado correctamente' />)
			setInterval(() => {
	...
	return (
		<>
			<div className='flex flex-col md:flex-row gap-y-5 w-full md:items-center justify-between my-10'>
				...
				<div>
					<SearchInput
						text='Ingresa el nombre del diseño...'
						...
						<>
							{data.map((item: DTF) => (
								<tr key={item.id} className='border-b border-mediumGray '>
									<th className='px-6 py-4'>{item.design_name}</th>
